package ec.edu.ups.ppw63.examenRecargas_CollaguazoDaniel_JEE.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/examenRecargas_CollaguazoDaniel_JEE.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "examenRecargas_CollaguazoDaniel_JEE.war").importFrom(f).as(WebArchive.class);

        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.ppw63.examenRecargas_CollaguazoDaniel_JEE.test");
